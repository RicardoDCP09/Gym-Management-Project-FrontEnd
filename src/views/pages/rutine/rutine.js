...
const Rutine = () => {
  ...
  const [newRutine, setNewRutine] = useState({
    name: '',
    difficulty: '',
    objective: '',
    coach_id: '',
  ...
  const handleEditRutine = async () => {
    ...
    try {
      const updateRutine = await API.put('routines', currentRutine, currentRutine.id_routine)
      setRutines((prevRut) =>
        prevRut.map((rutine) =>
          rutine.id_routine === currentRutine.id_routine ? { ...rutine, ...updateRutine } : rutine,
        ),
      )
  ...
  const getTraineer = (trainerId) => {
    const trainer = traineers
      ? traineers.find((traineer) => traineer.id_user === Number(trainerId))
      : null
  ...
  const getExercise = (exerciseId) => {
    const exercise = exercises
      ? exercises.find((exercise) => exercise.id_exercise === exerciseId)
      : null
  ...
  const getdifficulty = (difficultyId) => {
    const difficulties = difficulty
      ? difficulty.find((difficulty) => difficulty.id === difficultyId)
      : null
                ...
                <CRow>
                  <CCol className="mb-3" md={12}>
                    ...
                    >
                      ...
                      {traineers
                        .filter((traineer) => traineer.role === 2)
                        .map((traineer) => (
                ...
                <CTableDataCell>
                  ...
                  >
                      ...
                      <CRow className="mb-3">
                        <CCol md={6} className="mb-3">
                          <CFormInput
                            ...
                            onChange={(e) =>
                              setCurrentRutine({ ...currentRutine, name: e.target.value })
                            }
                          />
                        </CCol>
                        <CCol md={6} className="mb-3">
                          <CFormSelect
                            ...
                            onChange={(e) =>
                              setCurrentRutine({ ...currentRutine, difficulty: e.target.value })
                            }
                          >
                            <option value="">Select Difficulty</option>
                            {difficulty.map((difficulty) => (
                      ...
                      <CRow>
                        <CCol md={6} className="mb-3">
                          <CFormInput
                            ...
                            onChange={(e) =>
                              setCurrentRutine({ ...currentRutine, objective: e.target.value })
                            }
                          />
                        </CCol>
                        <CCol md={6} className="mb-3">
                          <CFormSelect
                            ...
                            onChange={(e) =>
                              setCurrentRutine({ ...currentRutine, exercise_id: e.target.value })
                            }
                          >
                            <option value="">Select a Exercise</option>
                            {exercises.map((exercise) => (
                      ...
                      <CRow>
                        <CCol md={12} className="mb-3">
                          <CFormSelect
                            ...
                            onChange={(e) =>
                              setCurrentRutine({ ...currentRutine, coach_id: e.target.value })
                            }
                          >
                            <option value="">Select a Traineer</option>
                            {traineers
                    ...
                    <CModalFooter>
                      ...
                      <CButton color="primary" onClick={handleEditRutine}>
                        Save Edit
                      </CButton>
                    </CModalFooter>
                  </CModal>
                  ...
                  >
                    ...
                    <CModalFooter>
                      ...
                      <CButton color="primary" onClick={handleDeleteRutine}>
                        Delete
                      </CButton>
                    </CModalFooter>
                  </CModal>
